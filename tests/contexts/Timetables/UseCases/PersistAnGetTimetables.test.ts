...
describe('Persist and get timetables use case', () =>
{
  ...
  beforeEach(() =>
  {
    ...
    repsolAlcudia = Timetables.timetablesInstancesFromString("L-D: 07:00-19:00", 13088);
  })
  test('Persist and get all timetables', async () =>
  {
    ...
    expect(allTimetables[1].fuelstationID).toBe(13088);
  })
  test('Persist and get timetables by fuel station ID', async () => {
    await inMemory.save([...cepsaAlaior, ...repsolAlcudia]);
    const repsolAlcudiatimetables = await inMemory.getByFuelstationID(13088);
    expect(repsolAlcudiatimetables.length).toBe(7);
    expect(repsolAlcudiatimetables[0].fuelstationID).toBe(13088);
